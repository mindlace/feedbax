...
| 3rd | 5 | bang | → `s imgbang`[obj-23] | fires bus `imgbang` (drives `feedbax.picsvid` — camera grab and housekeeping; not covered in this section). **It does not draw the sticker layer**: picsvid's `jit.gl.layer` [pv:obj-2] is `@automatic` (Sean's box has no `@automatic` key, so the ob3d default applies) and draws on the render bang, 7th below; `imgbang` reaches the layer only through `gate`[pv:obj-307], whose control toggle [pv:obj-316] is unfed and therefore closed. |
...
| 7th | 1 | bang | → jit.gl.render [obj-49] | a bare bang to the (`@automatic 0`) render context: performs the GL draw + buffer swap. The draw pass renders every `@automatic` child of context `foo` — in this patch that is the picsvid sticker/camera `jit.gl.layer` [pv:obj-2] — so the **sticker is composited *after* the plane (6th), alpha-blended on top of it** (`jit.gl.layer` enables blending and disables depth testing by default). That makes the sticker a convex stamp on the loop, bounded by its own brightness, whereas the waveform graphs (drawn 4th, *under* the plane) are added into the loop by the plane's `(SRC_ALPHA, DST_ALPHA)` composite. Post-retrofit (all children of `jit.gl.node fb` are `@automatic 1`) the same order is carried by `@layer`: sticker 20 > plane 10 > graphs 3 — see `docs/diagnosis-2026-08-23.md` finding J for what happens when the sticker sits under the plane. |
